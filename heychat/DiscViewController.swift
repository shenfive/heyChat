...
class DiscViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        discRef?.observe(.childAdded, with: { (dataSnapshot) in
                ...
                if let timestemp = dataSnapshot.childSnapshot(forPath: "timestemp").value as? Int{
                    discItem.timestemp = timestemp
                }
                ...
                self.discArray.sort(by: { (lhs, rhs) -> Bool in
                    return lhs.timestemp > rhs.timestemp
                })
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let date = Date.init(timeIntervalSince1970: Double(discItem.timestemp) / 1000)
    ...
    @IBAction func newMessage(_ sender: Any) {
        if let enteredMsg = msg.text{
            if enteredMsg.count > 0{
                ...
                let newMsg = ["content":enteredMsg,"nickname":nickname,"timestemp":ServerValue.timestamp()] as [String : Any]
